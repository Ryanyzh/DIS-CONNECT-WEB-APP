...
// Use a Proxy to provide a fallback for unknown ticket categories, so that if a new category is added in the future, it will still have a default badge style.
export const ticketCategoryConfig: Record<TicketCategory, string> = new Proxy(categoryBadgeMap, {
	get(target, key) {
		return typeof key === "string"
			? (target[key] ?? CATEGORY_BADGE_FALLBACK)

				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="grid gap-6 md:grid-cols-2">
						...
						<label className="space-y-2 text-body-md text-wise-ink dark:text-wise-canvas">
							...
							<select
								...
								disabled={!faculty}
								className="w-full rounded-wiseMd border border-wise-ink/20 bg-white px-4 py-3 pr-10 appearance-none text-body-md text-wise-ink outline-none transition focus:border-wise-green focus:ring-2 focus:ring-wise-green/20 dark:border-wise-mute dark:bg-wise-ink dark:text-wise-white disabled:cursor-not-allowed disabled:bg-wise-canvasSoft/50"
							>
								<option value="">
									{faculty ? "Select program" : "Choose faculty first"}
								</option>
								{programOptionsForFaculty.map((option) => (
									<option key={option} value={option}>
										{option}
									</option>
								))}
					...
					{notification && (
						<div
							role="status"
							aria-live="polite"
							className={`fixed top-6 right-6 z-[999] w-[min(100%,24rem)] rounded-wiseMd border px-4 py-3 text-body-md shadow-xl shadow-black/10 transition-all duration-300 ${

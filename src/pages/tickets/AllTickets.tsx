	...
	return (
		...
		>
			...
			<td className="p-0 w-1">
				<div className={`w-1 min-h-[44px] h-full ${ticket.isEscalated ? "bg-red-500" : statusStripeStyles[ticket.status]}`} />
			</td>

			{/* Ticket code + title */}
			<td className="px-4 py-3 max-w-[280px]">
				<div className="text-[10px] font-mono tracking-widest text-dc-text-muted leading-tight">{ticket.code}</div>
				<div className="text-sm font-semibold text-dc-text dark:text-white truncate mt-0.5">{ticket.title}</div>
			</td>

			{/* Category */}
			<td className="px-4 py-3 whitespace-nowrap">
				<span className={`inline-flex items-center text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${categoryStyles[ticket.category]}`}>
					{ticket.category}
				</span>
			</td>
			...
			<td className="px-4 py-3 whitespace-nowrap">
				{ticket.officer ? (
					<span className={`text-xs ${ticket.isEscalated ? "text-dc-error dark:text-dc-error-dark font-medium" : "text-dc-text-muted"}`}>
						{ticket.officer.name}
					</span>
				) : (
			...
			{/* Priority */}
			<td className="px-4 py-3 whitespace-nowrap">
				<span className={`inline-flex items-center text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${priorityStyles[ticket.priority]}`}>
			...
			{/* Status */}
			<td className="px-4 py-3 whitespace-nowrap">
				<span className={`inline-flex items-center text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${statusStyles[ticket.status].text} ${statusStyles[ticket.status].bg}`}>
					{ticket.status}
				</span>
			</td>
		</tr>
...
function IconList({ active }: { active: boolean }) {
	return (
		<svg className={`w-4 h-4 transition-colors ${active ? "text-dc-primary" : "text-dc-text-muted"}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
...
function IconTable({ active }: { active: boolean }) {
	return (
		<svg className={`w-4 h-4 transition-colors ${active ? "text-dc-primary" : "text-dc-text-muted"}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
			<path strokeLinecap="round" strokeLinejoin="round" d="M3 5h18M3 9h18M3 13h18M3 17h18M9 5v14M15 5v14" />
		</svg>
	);
}
		...
		<PageShell title="All Tickets" description="View every ticket in the system and filter by status, category, or owner.">
			<div className="flex flex-col gap-4">
					...
					<div className="grid grid-cols-6 divide-x divide-dc-border dark:divide-zinc-800">
						{stats.map(({ status, count }) => (
							...
							>
								...
								<span className="text-[11px] text-dc-text-muted mt-0.5 text-center leading-tight">{status}</span>
							</button>
						))}
					</div>
				...
				<div className="bg-dc-surface dark:bg-zinc-900 border border-dc-border dark:border-zinc-800 rounded-xl px-4 py-2.5 flex items-center gap-3">
					...
					<div className="ml-auto flex items-center gap-3">
						<span className="text-xs text-dc-text-muted hidden sm:block">
							<span className="font-semibold text-dc-text dark:text-white">{filteredTickets.length}</span>
						...
						<div className="flex items-center border border-dc-border dark:border-zinc-800 rounded-lg overflow-hidden">
							{(["list", "card", "table"] as ViewMode[]).map((mode, i) => (
								...
								>
									...
									{mode === "table" && <IconTable active={viewMode === "table"} />}
								</button>
							))}
						</div>
					</div>
				...
				{filteredTickets.length === 0 ? (
					<p className="text-sm text-dc-text-muted py-10 text-center">No tickets match the current filters.</p>
				) : (
					<>
						{/* LIST VIEW */}
						{viewMode === "list" && (
							<div className="flex flex-col">
								{/* Column header — pl-5 aligns with list card content (1px border + 4px stripe + 16px px-4) */}
								<div
						...
						{/* CARD VIEW */}
						{viewMode === "card" && (
							<div className="grid grid-cols-2 xl:grid-cols-3 gap-3 max-h-[65vh] overflow-y-auto pr-1">
								{filteredTickets.map((ticket) => (
